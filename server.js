...
// Needs yt-dlp + ffmpeg + the bgutil PO-token provider (see Dockerfile). Public content only: no cookies, no logins.
...
// Node is bundled in this image and used both for yt-dlp's own signature decryption (--js-runtimes)
// and, via the bgutil server running alongside this process on 127.0.0.1:4416, for minting YouTube
// proof-of-origin tokens so requests from this server's IP are trusted without cookies/login.
// Deliberately NOT forcing a specific --extractor-args player_client list here: YouTube's accepted
// client set changes frequently (it broke a hardcoded web_safari/mweb/tv override in mid-Sept 2026),
// so we let yt-dlp pick its own current default and rely on the PO token to make it trusted.
...
function ytdlpJSON(url){return new Promise((ok,no)=>execFile('yt-dlp',['-J','--no-playlist','--no-warnings',...COMMON_ARGS,url],{timeout:60000,maxBuffer:64e6},(e,out,err)=>{
...
async function info(url){
  try{return buildInfo(await ytdlpJSON(url))}
  catch(e){
    // A 'bot' result right after the PO-token server started (cache still warming up) is often transient.
    if(why(e.message)==='bot'){
      await new Promise(res=>setTimeout(res,1500));
      try{return buildInfo(await ytdlpJSON(url))}
      catch(e2){console.error('[info]',url,e2.message);throw new Error(why(e2.message))}
    }
    console.error('[info]',url,e.message);throw new Error(why(e.message));
  }
}
...
function buildArgs(url,type,h,lang,base){
  const a=['--no-playlist','--no-warnings',...COMMON_ARGS];
...
async function prepare(id,url,type,h,lang,title){
  ...
  let lastErr=null;
  try{await runDownload(buildArgs(url,type,h,lang,base))}
  catch(e){
    lastErr=e;
    if(why(e.message)==='bot'){
      await new Promise(res=>setTimeout(res,1500));
      try{await runDownload(buildArgs(url,type,h,lang,base));lastErr=null}catch(e2){lastErr=e2}
    }
  }
  running--;
