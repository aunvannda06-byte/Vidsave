FROM node:22-slim
RUN apt-get update && apt-get install -y --no-install-recommends python3 python3-pip ffmpeg ca-certificates git \
...
CMD ["sh","-c","pip3 install --break-system-packages -q -U yt-dlp bgutil-ytdlp-pot-provider || true; node /opt/bgutil/server/build/main.js & node server.js"]
