...
RUN apt-get update && apt-get install -y --no-install-recommends \
 ...
 && pip3 install --break-system-packages -U --pre yt-dlp bgutil-ytdlp-pot-provider \
...
CMD ["sh","-c","pip3 install --break-system-packages -q -U --pre yt-dlp bgutil-ytdlp-pot-provider || true; (node /opt/bgutil/server/build/main.js || echo '[bgutil] PO-token server exited/failed to start') & node server.js"]
